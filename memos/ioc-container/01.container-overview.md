...
* `Lifecycle`インタフェース (`start()`, `stop()`, `boolean isRunning()`)
* `LifecycleProcessor`インタフェース (`Lifecycle` + `onRefresh()`, `onClose()`)
* `SmartLifecycle`インタフェース (`Lifecycle` + `int getPhase()`, `boolean isAutoStartup()`, `stop(Runnable callback)`)

...
| Class | Description |
| ----- | ----------- |
| `org.springframework.beans.factory.BeanFactory` | IoC Containerの基本的な機能を提供するインタフェースです。 |
| `org.springframework.context.ApplicationContext` | `BeanFactory`のスーパーセット(`BeanFactory`の子インタフェース)で、エンタープライズ向けアプリケーションで必要となる機能(AOP, メッセージ管理, イベント通知など)が追加されているインタフェースです。 |

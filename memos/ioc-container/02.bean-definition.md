...
>
> `DisposableBean#destroy()`と`<bean>`要素の`destroy-method`属性 (`@Bean`の`destroyMethod`属性)の指定がないケースにかぎり、`java.lang.AutoCloseable`又は`java.io.Closeable`を実装すると`close()`メソッドが呼び出されます。
> これは、明示的に破棄メソッドを指定しなくても、`java.lang.AutoCloseable`又は`java.io.Closeable`を実装していれば破棄処理が実行されることを意味しています。
